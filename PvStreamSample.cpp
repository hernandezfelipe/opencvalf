...
// VARIÁVEIS CRIADAS PARA TESTE
cv::Mat joinedFrame; //CARCAÇA DE ARRAY PARA CONCATENAR
unsigned int totalFrames = 100; //NÚMERO DE VEZES QUE A CÂMERA ADQUIRE ANTE DE SOLTAR A IMAGEM
unsigned int currentFrame = 0; //VARIÁVEL INCREMENTAL PARA CONTAGEM
bool imageTaken = false; //BOOLEAN PARA ENCERRAMENTO DO CÓDIGO
        ...
        if ( lResult.IsOK() )
        {
            if ( lOperationResult.IsOK() )
            {
                ...
                if ( lType == PvPayloadTypeImage )
                {
					...
					if (cv::countNonZero(workImage) < 1) { //TESTA QUANTIDADE DE NÃO-ZEROS (PIXELS PRETOS)

						// IGNORA MATRIZES TODAS EM PRETO ATÉ O STREAM COMEÇAR (POR ALGUM MOTIVO ELE CAPTURAVA ALGUMAS LINHAS TODAS EM PRETO ANTES DE AQUECER O SISTEMA)
					}
					else if ( currentFrame == 0 ) //SE O FRAME ATUAL É O PRIMEIRO (NÚMERO 0)
					{
						currentFrame++; // INCREMENTA
						joinedFrame = workImage; // COLOCA O ARRAY PEGO NA VARIÁVEL JOINEDFRAME
						cout << "Frame: " << currentFrame << " de " << totalFrames << endl; // MOSTRA O PROGRESSO
					}
					else if ( currentFrame++ < totalFrames ) // SE O FRAME ATUAL É MAIOR QUE ZERO E MENOR QUE O MÁXIMO
					{
						cout << "Frame: " << currentFrame << " de " << totalFrames << endl; // MOSTRA O PROGRESSO 
					...
					{
						currentFrame = 0; // QUANDO ROMPE, ZERA A VARIÁVEL
